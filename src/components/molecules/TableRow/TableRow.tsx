...
export interface TableRowProps {
  ...
  cells?: Array<{
    ...
    content: 
      ...
      | { type: 'chip'; label: string; variant?: 'primary' | 'success' | 'warning' | 'error' | 'info' };
    align?: string;
  }>;
}
...
export const TableRow: React.FC<TableRowProps> = ({
  ...
  return (
    <div className={classNames('grid grid-cols-6 gap-4 items-center py-4 px-6', className)}>
      {/* ID Column */}
      ...
      {cells && cells.map((cell, index) => (
        <div key={index} style={{ width: cell.width }} className={classNames(cell.align)}>
          ...
          {cell.content.type === 'chip' && (
            <Chip variant={cell.content.variant || 'primary'}>{cell.content.label}</Chip>
          )}
        </div>
      ))}
    </div>
